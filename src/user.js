...
function renderLogin() {
  if (sessionStorage.getItem("username")) {
    ...
    let inputGroup = createHtmlElement(
      "form",
      "input-group",
      "",
      "input-group-1"
    ...
    let loginButton = createHtmlElement(
      "input",
      "button",
      "Login / Sign Up",
      "login-button"
...
function renderUserBar(userInfo) {
  ...
  let logoutButton = createHtmlElement(
    "div",
    "col-2 btn btn-dark",
    "Logout",
    "logout-button"
...
function updateAccount(amount) {
  ...
  return fetch(
    `https://blueberry-pudding-46059.herokuapp.com/api/v1/users/${user}`,
    {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
    ...
    .then(json => {
      ...
      document.getElementById("user-information").textContent = `${
        json.username
      } $${json.balance}`;
...
function zeroBalance() {
  if (parseInt(sessionStorage.getItem("balance")) == 0) {
    document.getElementById("result").textContent =
      "Please Add More Chips to Continue";
    let addMoreChips = createHtmlElement(
      "button",
      "col-2 bg-secondary",
      "Add Chips",
      "add-chips"
...
function updateWinPercentage(array) {
  ...
  if (array.length > 0) {
    ...
    document.getElementById(
      "win-percentage"
    ).textContent = `Win Percentage ${Math.ceil((wins / totalHands) * 100)}%`;
  }
}
